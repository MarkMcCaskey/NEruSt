...
// $09
pub fn ora_imm(cpu: &mut CPU, val: u8) {
    set_acc_ora_conditionally_zero_flag(cpu, val);
}

// $05
pub fn ora_zp(cpu: &mut CPU, ram: &RAM, adr: u8) {
    let val = ram.data[adr as usize];
    set_acc_ora_conditionally_zero_flag(cpu, val);
}

// $15
pub fn ora_zpx(cpu: &mut CPU, ram: &RAM, adr: u8) {
    let val = ram.data[((adr as u16) + (cpu.x as u16)) as usize];
    set_acc_ora_conditionally_zero_flag(cpu, val);
}

// $0D
pub fn ora_abs(cpu: &mut CPU, ram: &RAM, adr: u16) {
    let val = ram.data[adr as usize];
    set_acc_ora_conditionally_zero_flag(cpu, val);
}

// $1D
pub fn ora_abx(cpu: &mut CPU, ram: &RAM, adr: u16) {
    let val = ram.data[(adr + (cpu.x as u16)) as usize];
    set_acc_ora_conditionally_zero_flag(cpu, val);
}

// $19
pub fn ora_aby(cpu: &mut CPU, ram: &RAM, adr: u16) {
    let val = ram.data[(adr + (cpu.y as u16)) as usize];
    set_acc_ora_conditionally_zero_flag(cpu, val);
}

// $01
pub fn ora_izx(cpu: &mut CPU, ram: &RAM, adr: u8) {
    let val = ram.data[(ram.data[((adr as u16) + (cpu.x as u16)) as usize]) as usize];
    set_acc_ora_conditionally_zero_flag(cpu, val);
}

// $11
pub fn ora_izy(cpu: &mut CPU, ram: &RAM, adr: u8) {
